...
The representative cross-file POC converts a Fleeting Note into a new `backlog` / `medium` Task with a new UUID, confirms the target write, removes the source record, and compensates the created Task if source removal fails. The same cross-file executor now powers Fleeting Note Archive, Delete to Trash, and Restore while preserving the original Fleeting Note UUID and `created` timestamp. The lifecycle page uses card-based Active, Archived, and Trash sections, keeps each section ordered by `created`, and exposes pending, error, and `Review required` states. Conversion success, `unchanged`, `compensated`, and `partial` paths have been verified end to end in Windows Desktop Obsidian. Lifecycle Archive, Delete, Restore, stable display ordering, a representative `partial` path, duplicate lifecycle UUID reporting, manual recovery, and queue continuation have also been verified in the real host. Runtime Store reconciliation always reflected the final disk state; the lifecycle `partial` case intentionally exposed both records and a duplicate UUID Data issue instead of hiding the partially completed result.
...
The Fleeting Notes page reads the active, archived, and deleted lifecycle files. Creating or editing a valid top-level record in any managed lifecycle file should update the open page automatically. To test conversion, select a target Project and click `Convert to Task`; the Note should disappear only after the queued cross-file Mutation completes, the new `backlog` / `medium` Task should appear in the selected Project, and no transient Data issues should appear. Archive should move an active Note to `Trail/Archive/Fleeting Notes.md`; Delete should move it to `Trail/Trash/Fleeting Notes.md`; Restore should return it to the active file while preserving its UUID and `created` timestamp. The UI remains ordered by `created` even though restored records are appended physically. Remove local lifecycle test files before committing.
