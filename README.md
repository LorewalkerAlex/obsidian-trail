...
The current POC has established the plugin shell, Markdown reading, precise Task status write-back, a plugin-level Runtime Store with a mutation refresh boundary, a plugin-level generic global Mutation Queue, Vault file-event reconciliation, and a representative cross-file Mutation. Task status changes and Fleeting Note-to-Task conversion are both routed through the plugin entrypoint. Trail discovers checked-in fixtures under `Trail/`, parses Area, Project, Task, Subtask, Task Note, Project Note, and Fleeting Note data, and renders the confirmed Store snapshot in a single Trail `ItemView`. `Trail/Fleeting Notes.md` is part of the Vault read path, Runtime Store snapshot, file-event reconciliation, and a minimal interactive Fleeting Notes page.

The plugin listens for relevant `create`, `modify`, `delete`, and `rename` events under `Trail/Areas/` and for the managed `Trail/Fleeting Notes.md` file, debounces refreshes, and ignores unrelated or nested files outside the managed scope. External Project changes and Fleeting Note edits update an open Trail view without reopening it. Area and Project frontmatter are parsed from the same `Vault.cachedRead()` Markdown snapshot as the body, avoiding a post-Mutation race with stale `MetadataCache` data. While a queued Mutation is running, the Runtime Store suppresses file-event refreshes and performs one final reconciliation after the Mutation finishes, including failure paths.
...
The representative cross-file POC converts a Fleeting Note into a new `backlog` / `medium` Task with a new UUID, confirms the target write, removes the source record, and compensates the created Task if source removal fails. The Fleeting Notes page provides a target Project selector and a per-Note `Convert to Task` action. Conversion runs through the plugin-level serial queue and Runtime Store Mutation boundary, shows pending and error feedback, and blocks direct retry after a `partial` result. The success, `unchanged`, `compensated`, and `partial` paths have been verified end to end in Windows Desktop Obsidian through temporary host-level fault injection. Every outcome reconciled the Runtime Store to the final disk state without transient Data issues; `partial` blocked direct retry, manual recovery restored a normal conversion without reloading the plugin, and the queue continued processing subsequent commands.
...
The Fleeting Notes page reads `Trail/Fleeting Notes.md`. Creating, editing, or deleting a valid top-level Fleeting Note should update the open page automatically and should not create Data issues. To test conversion, select a target Project and click `Convert to Task`; the Note should disappear only after the queued cross-file Mutation completes, the new `backlog` / `medium` Task should appear in the selected Project, and no transient Data issues should appear. Restore the checked-in fixture and remove the local Fleeting Notes test file before committing.
