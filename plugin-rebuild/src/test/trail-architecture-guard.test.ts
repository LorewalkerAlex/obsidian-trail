...
const SOURCE_ROOT = join(process.cwd(), "plugin-rebuild", "src");
...
describe("Trail rebuild architecture guards", () => {
  const allSourceFiles = collectTypeScriptFiles(SOURCE_ROOT);
  const productionFiles = allSourceFiles.filter((path) => !isTestFile(path));

  it("keeps migration-stage identity out of rebuild production symbols and runtime text", () => {
    const stageLanguage = [
      /\bFormal\b/,
      /[A-Za-z]Formal[A-Z]/,
      /formal-/,
  ...
  it("keeps the production bundle entry on the rebuild composition root", () => {
    const buildConfig = readFileSync(BUILD_CONFIG, "utf8");

    expect(buildConfig).toContain('entryPoints: ["plugin-rebuild/src/main.ts"]');
    expect(buildConfig).not.toContain('entryPoints: ["plugin/src/main.ts"]');
  });
});
