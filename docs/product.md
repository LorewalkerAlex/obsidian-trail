...
A fresh Workspace seeds one normal Project titled `Standalone` at `Projects/0000 Standalone.md` and makes that Project the initial Default Project. The seed uses normal Project creation/default-Status semantics and may later be renamed, assigned to an Initiative, moved through any legal Project lifecycle transition, given Milestones and properties, or deleted after another existing Project has first become the Workspace Default.
...
If persisted Workspace State has no `defaultProjectId` at startup, Trail performs one narrow recovery before normal ready state. Source Sync inspects the reserved Project sequence `0000`, not a title: if one valid ordinary Project carrier occupies that sequence, Trail records its stable ID regardless of the carrier's current readable suffix/title; if no `0000` Project carrier exists, recovery creates the standard `Projects/0000 Standalone.md` Project and records its new stable ID. An existing but invalid or otherwise untrusted `0000` carrier is a Data Issue and is not overwritten. This recovery does not make sequence, path, or title into Project identity and is not a general fallback lookup rule.

The current Default Project is not a legal Project Delete target. To delete it, the user first chooses another existing Project as the Workspace Default and lets that independent Workspace State change commit; the former Default can then be deleted through the ordinary Project Delete flow. Project Delete does not accept or infer a replacement Default, and Trail never intentionally publishes a normal ready Workspace without one.
...
Closing a Cycle is explicit. An ordinary Close records the actual end and freezes the membership that is still present when Close commits, without changing any member's Status, Project, Milestone, Priority, Estimate, Labels, or Due. Reaching the planned end never auto-closes the Cycle.

`Close and start next` is a separate explicit planning flow rather than a close-first shortcut. The source Cycle remains Open while the successor scope is prepared. Currently unfinished source members may be preselected as successor candidates, but the user may deselect any of them, add other legal Workflow Issues, or cancel. When the user confirms, any selected successor member that is still in the source Cycle is transferred out of the source membership before the source closes; the source then freezes its remaining membership, and the successor starts with the full confirmed selection. Issues selected for the successor that were not members of the source are simply added to the successor. Cancel leaves the source Cycle Open and unchanged. The transfer changes only Cycle membership and does not change Issue facts. This is explicit user-selected rollover, not automatic rollover.

Historical Cycles are passive membership history. Their final Issue membership is the membership actually present when Close commits after any explicit pre-close transfer. V1 does not snapshot Issue Status, Project, Estimate, Progress, or other analytics at close. A Historical Cycle opens as a flat List-only collection whose rows resolve the member Issues' current live fields; Status and Project are displayed as ordinary row metadata rather than grouping the collection. Historical membership cannot be normally edited.
...
9. **Default Project is required routing state, not a Project subtype.** A fresh Workspace seeds `Standalone` as an ordinary Project and references it for default selection/navigation; startup repairs only a missing persisted reference before normal ready state. The current Default cannot be deleted: another existing Project must first become Default through a separate Workspace mutation, after which the former Default may be deleted normally.
10. **Cycle planning is explicit, selection-based, and independent of Workflow lifecycle.** An Open Cycle may select any Workflow Issue; joining/leaving a Cycle changes only Cycle membership, and Issue Status/Project changes never silently add or remove membership. Ordinary Close retains the membership present at close. `Close and start next` may explicitly transfer user-selected overlapping members out of the source before it closes, so Historical Cycles retain the membership actually present after that transfer rather than duplicating transferred work. Cycle Progress and Effort remain live derived projections; no close-time Issue-property snapshot is introduced.
...
12. **Drag has one global meaning for Workflow Issues on supported Status-grouped List/Board surfaces: Status change.** Relationship changes use explicit actions. A selected same-Status set may move together as one drag scope; a selection spanning multiple concrete Statuses is not a multi-item drag scope.
