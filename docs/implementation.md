...
- **formal UI implementation gap** — the clean active-tree scaffold, Foundation Lab, frontend architecture/foundation consolidation, Visual Foundation acceptance, and independently justified Core Primitive Kit now exist. Active work is the Shared Pattern + Interaction Kit: the View Bar and CollectionRow implementation candidates are automated-check green and host-calibrated but still need non-Foundation production collection consumers for Rule-9 acceptance. CollectionRow currently owns generic row presentation plus an independent selection gutter/presentation state only; Selection lifetime, range/select-all, Action Registry, Bulk, and keyboard dispatch remain unresolved implementation work in the later shared interaction owner. After that come the remaining shared pattern/interaction and semantic component stack, Location/View Bars, shared Creation Composer, simplified Filter/session state, Selection/Action system, Project/Initiative/Triage/Cycle/Home/Search composition, Workspace Grid, Inspector/Peek ownership, Linear-like Runtime feedback, Default Project settings interaction, and later consumer-specific responsive calibration on that single architecture;
...
- [ ] **5. Shared Pattern + Interaction Kit**
  ...
  - remaining Surface/Card foundations, PropertyControl shell, Overlay/Composer shell, List/Board foundations, and common focus/selection mechanics beyond CollectionRow presentation;
...
The View Bar and CollectionRow implementation candidates are not yet counted as completed Gate 8 slices because their checked-in/runtime consumer graph still ends at Foundation Lab; Rule 9 acceptance waits for non-Foundation production collection consumers to exercise the contracts.
...
### 6.17 General verification discipline
