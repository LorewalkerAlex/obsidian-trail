...
### 4.5 Frontend architecture and Visual Foundation accepted; Core Primitive Kit in progress

The bounded V1 interaction design remains frozen in `ui.md`. The deliberate UI Reset, Foundation implementation, frontend architecture consolidation, and Visual Foundation acceptance are complete enough for production primitives to consume the shared visual contracts without treating CSS cascade history or Lab specimens as production APIs.
...
- [ ] **4. Core Primitive Kit**
  ...
  - remaining kit: Badge/Tag where justified, Tooltip, Popover, Menu, Select/Combobox, Dialog, Kbd, and common loading/disabled/destructive states;
...
- Core Primitive Kit third slice **(native Checkbox contract only; collection-selection behavior remains shared-pattern work and the stage remains open)**.
...
This closes only the native Checkbox primitive contract as the third Core Primitive Kit slice. Selection state, hover-to-checkbox replacement, selected-row presentation, deselection/range/select-all/Bulk behavior, and any justified indeterminate state remain owned by the future CollectionRow + shared Selection pattern rather than this primitive.

### 6.14 General verification discipline
...
V1 implementation is ready for final product hardening only after the frozen Product/Domain/Data/UI answers are implemented through their canonical owners without temporary models, alternate persistence paths, duplicate mechanisms, or Page-private reconstructions. The exposed lower-layer semantic corrections through Milestone Progress, the UI Reset, frontend architecture/Foundation consolidation, and Visual Foundation acceptance are closed. The legacy POC presentation has already left the active implementation; before final hardening, the accepted token foundation must now complete the remaining Core Primitive Kit -> pattern/interaction -> semantic component -> page/shell stack and the remaining product locations plus Default Project Settings/delete guidance. Cycle Start/Close/Start-next interaction/vocabulary, Milestone Progress semantics, the Required Default Project ready-state lower layer, Canonical Estimate alignment, and Workflow Issue Project-control legality are already closed.
