...
- [x] **3. Visual Foundation acceptance**
  - accepted canvas/surface hierarchy, typography, accent/semantic colors, spacing/density direction, radius, hairlines/elevation, icon sizing/stroke rhythm, focus/hover/pressed/selected/disabled/destructive/text-selection states, restrained motion principle, and document/editor presentation;
...
- [ ] **5. Shared Pattern + Interaction Kit**
  - [ ] first bounded View Bar candidate: responsive Filter/Display action slots and optional binary List/Board/Timeline layout switching are implementation/host-calibration green; final acceptance waits for the first non-Foundation production collection consumer;
  - remaining Surface, CollectionRow/Card foundations, PropertyControl shell, Overlay/Composer shell, List/Board foundations, and common focus/selection presentation;
...
The published View Bar implementation candidate is not yet counted as a completed Gate 8 slice because its only checked-in runtime consumer is Foundation Lab; Rule 9 acceptance waits for the first non-Foundation production collection consumer.
...
- Foundation Lab consumes the real primitive inside the Collection density leading slot rather than as an isolated form specimen: the ordinary row keeps its semantic glyph, the demo-hover row exposes an interactive unchecked Checkbox, and the selected calibration row shows a checked read-only Checkbox;
- the Lab's selected checkbox is intentionally read-only because the Lab is demonstrating three presentation states, not pretending to implement the future CollectionRow + Selection state machine; the real selected row must remain deselectable once that shared pattern exists;
...
This closes the native Checkbox primitive contract as the third Core Primitive Kit slice. Selection state, hover-to-checkbox replacement, selected-row presentation, deselection/range/select-all/Bulk behavior, and any justified indeterminate state remain owned by the future CollectionRow + shared Selection pattern rather than this primitive. Together with the first two slices, this is the accepted independently justified Core Primitive Kit foundation; later generic capabilities are introduced from real shared-pattern/semantic consumers rather than as prerequisite empty components.
...
### 6.15 General verification discipline
