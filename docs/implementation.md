...
- Delete is exposed through the existing Issue Application intent and shared Confirmation. Confirmation remains the top transient layer; cancel/Esc restores focus to the surviving interaction context. Selected-row, Peek, and Bulk paths return to their surviving trigger, while explicit unselected-row Delete returns to the Project Page surface so dismissal does not manufacture row focus-visible selection chrome;
