...
- established Cycle Start/Close/Close-and-start-next owners over the shared Mutation infrastructure;
...
| Progress | Accepted / Lab-proven / Consumer-proven / Host-proven | preserve the shared normal/compact/micro/unavailable owner across Project, Cycle, and Home during Stage 12 calibration |
| Home Read Model / Page + creation action | Accepted / Consumer-proven / Host-proven | preserve modular Home composition, shared Work Pulse owners, Weekly Update utility ownership, and standard creation routing during Stage 12; no Home-specific creation system |
...
- **Home temporal Read Model and first Product consumer are Consumer-proven / Host-proven** — `selectTrailHomeReadModel` acquires one readable Runtime snapshot plus explicit `now`, applies Workspace timezone semantics, and derives the Monday-Sunday This Week projection plus rolling three-calendar-month Lifecycle Activity and Work Trend projections. The real Home Page consumes the same production widgets already exercised in Foundation rather than introducing Page-local copies.
...
- **Work Trend projection is Consumer-proven** — the real Home Read Model derives Backlog stock, Active stock, and rolling seven-day Completed throughput from retained Workflow Issue lifecycle facts, while the accepted 2x1 widget keeps the shared-axis stock/throughput grammar and hover/focus exact-value detail. The larger 2x2 study remains retired from the active specimen set.
- **Host tooltip ownership is resolved at the shared host boundary** — Trail-owned Main View, Navigation, Inspector, Composer, Confirmation, and Popover surfaces suppress Obsidian automatic visual tooltips by default while preserving accessible naming. Visual tooltip/detail presentation is explicit opt-in. Representative real-Obsidian verification confirms Work Trend keeps only its own date/value detail, Trail controls no longer duplicate host tooltips, and native Obsidian chrome outside Trail remains unaffected.
- **Temporal-consumer validation is green** — focused Home Query/Page/Foundation/Shell coverage passes 4 test files / 9 tests, with lint plus diagnostics typecheck/build green. Representative real-Obsidian verification confirms the real Home composition and the accepted low-noise shared widget frame treatment.
- **Weekly Meeting Notes is Consumer-proven / Host-proven** — Home now consumes the existing `Weekly Update.md` utility through the established Weekly Note Application rather than creating Domain/Runtime state. The 2x1 module owns Current read mode, module-local edit draft, internal History, manual Archive/next, and structural H1/H2 feedback. Representative real-Obsidian verification confirms the empty/read and edit/error compositions; persistence save/archive behavior remains covered by the existing utility repository/Application plus widget/Page integration tests rather than requiring a destructive host mutation.
- **Weekly-notes integration validation is green** — focused utility-persistence/Application/widget/Home/Foundation/Shell coverage passes 6 test files / 25 tests, with ESLint, diagnostics typecheck/build, and diff sanity green.
- **Work Pulse is Consumer-proven / Host-proven** — the Home Read Model now derives Current Cycle, active Triage pressure, and Started-category Project summaries from the same coherent readable Runtime snapshot. Wide/normal Home pairs the compact This Week module with a content-dense Work Pulse summary; constrained composition reflows by Main View capacity without inventing new metrics.
- **Work Pulse reuse and navigation ownership is closed** — Cycle and Project summaries reuse shared Progress, Home Triage reuses the same Segmented Summary production owner as Project Attention with an inline presentation, and the navigable Cycle/Triage regions plus visible Project rows expose full-region hit targets while preserving Projects Root access from the section heading/overflow.
- **Work Pulse validation is green** — the closing shared-owner coverage passes 5 test files / 10 tests, with ESLint, diagnostics typecheck/build, and diff sanity green. Representative real-Obsidian verification covers wide and narrower Home composition plus whole-region Cycle/Triage/Project navigation.
...
Stages 0 through 11 are closed. Stage 12A.1 baseline cleanup/documentation, Stage 12A.2 longitudinal fixture, and Stage 12A.3 whole-product E2E regression are complete; the published fixture baseline is `46eb8694c61cd18a467418e4900ca34fb44ffa1f`. The published `eeeb85e8ab3de653bcdfc42f1babac40b5575f7a` checkpoint fixes Issue Inspector Status changes so an existing Estimate is retained without being resubmitted as illegal non-Completed input. Real-Obsidian E2E is green across Triage -> Issue acceptance, Full Item/Inspector editing, Issue lifecycle reopen/complete behavior, Project List/Board drag, Filter, Peek/Selection, Sidebar Search, native Back/Forward, Current/Historical Cycle collection behavior, Projects/Initiative navigation, Composer dismissal, destructive confirmation, Triage Delete, and the repaired Cycle rollover lifecycle. The affected Cycle rerun proves Start-next planning leaves the source Open, Cancel submits no Cycle mutation, and confirmed partial transfer commits one `planning.cycle.close-and-start-next` replace+create intent: the source closes with only the untransferred plus already-terminal members, the successor becomes Current with the 12 confirmed transfers, and no Workflow Issue source is changed. Stage 12A functional acceptance is therefore green. Stage 12B full-system visual/UI calibration is now active; the unselected-row Delete -> Esc focus-restoration checkbox appearance plus the previously deferred Project lifecycle/Bulk/Delete transient micro-calibration remain calibration targets rather than functional blockers.
