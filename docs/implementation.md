...
Required Workflow Project & Default Project and Project Status Four-State Configuration Closure are published implementation checkpoints. Project Workspace/Projects Root, Initiative Focus, Triage, Cycle, Creation Surface, simplified shared Filter, shared Selection/Action design, Home, Workspace Grid/responsive composition, Search, Runtime/Data-Issue/optimistic feedback, and the Default Project setter are resolved at the target-design level, so Gate 8 may treat the V1 UI authority as frozen. The exposed lower-layer semantic corrections through Milestone Progress are closed. The UI Reset, Foundation Lab bootstrap, frontend architecture consolidation, and Visual Foundation acceptance are now locally green against the real Obsidian host. Reusable visual facts have one token/contract authority, representative native Obsidian consumers and Trail calibration surfaces share the accepted Linear-faithful application presentation, and values that still require a real production consumer remain explicitly replaceable rather than prematurely frozen. The independently justified Core Primitive Kit foundation is now accepted through its first three bounded slices: Button/IconButton + Input/Textarea, Progress/Separator, and Checkbox. The active implementation boundary is the Shared Pattern + Interaction Kit. Its View Bar, CollectionRow, and PropertyControl implementation candidates are automated-check green and host-calibrated in real Obsidian. View Bar owns the reusable collection-control plane; CollectionRow owns generic dense-row geometry plus independent selection-gutter and selected/highlight presentation; PropertyControl owns only the compact button-like property-control presentation shell and its proven compact density. Selection lifetime/range/select-all/action semantics, property-specific meaning, picker/open state, capability legality, and mutation behavior remain with later interaction/semantic owners. Rule 9 keeps all three candidates unaccepted until non-Foundation production consumers exercise their contracts. Additional generic primitive names are not a prerequisite checklist; Badge/Tag, Tooltip, Popover, Menu, Select/Combobox, Dialog, Kbd, and richer shared states are introduced only when a concrete pattern/interaction or semantic consumer proves the contract. The remaining UI gaps are recorded in Section 4.4.
...
- **formal UI implementation gap** — the clean active-tree scaffold, Foundation Lab, frontend architecture/foundation consolidation, Visual Foundation acceptance, and independently justified Core Primitive Kit now exist. Active work is the Shared Pattern + Interaction Kit: the View Bar, CollectionRow, and PropertyControl implementation candidates are automated-check green and host-calibrated but still need non-Foundation production consumers for Rule-9 acceptance. CollectionRow currently owns generic row presentation plus an independent selection gutter/presentation state only; PropertyControl owns only compact property-control presentation and one proven compact density. Selection lifetime, range/select-all, Action Registry, Bulk, keyboard dispatch, property semantics, picker state, capability legality, and mutation behavior remain unresolved work at their later shared interaction/semantic owners. After that come the remaining shared pattern/interaction and semantic component stack, Location/View Bars, shared Creation Composer, simplified Filter/session state, Selection/Action system, Project/Initiative/Triage/Cycle/Home/Search composition, Workspace Grid, Inspector/Peek ownership, Linear-like Runtime feedback, Default Project settings interaction, and later consumer-specific responsive calibration on that single architecture;
...
### 4.5 Core Primitive Kit accepted; Shared Pattern + Interaction Kit in progress

The bounded V1 interaction design remains frozen in `ui.md`. The deliberate UI Reset, Foundation implementation, frontend architecture consolidation, and Visual Foundation acceptance are complete enough for production primitives and shared patterns to consume the shared visual contracts without treating CSS cascade history or Lab specimens as production APIs.
...
The first bounded Shared Pattern + Interaction Kit implementation candidate now provides production `TrailViewBar`, `TrailViewBarAction`, and `TrailViewLayoutSwitch` owners under `ui/patterns`. `TrailViewBar` owns one collection-control plane with a flexible leading region and a compact trailing cluster; Filter and Display remain composable action slots, while List-only consumers omit the optional layout slot. `TrailViewLayoutSwitch` accepts exactly two caller-supplied layout options and expresses current state through button semantics plus `aria-pressed`; it reuses `TrailIconButton` for icon-only mechanics instead of becoming a generic segmented form control. The pattern owns relative composition rather than page coordinates: the leading region flexes and wraps, the trailing cluster keeps intrinsic width and pushes to the available edge, and the bar can wrap when pane capacity no longer supports one row. No window-width breakpoint or Foundation-Lab position is part of the production contract.

Foundation Lab now consumes the complete production View Bar rather than displaying isolated Filter/Display/layout specimens. Its local List/Board state exists only to exercise the callable pattern contract. Full local verification is green across 95 Vitest files / 326 tests, ESLint, TypeScript, production esbuild, and `git diff --check`. Representative real-Obsidian validation exercised both List and Board selected states across wide and narrow Trail panes: the leading Filter action and trailing layout/Display cluster retain their hierarchy, the selected layout remains a quiet state rather than a capsule control, and narrowing the pane compresses the available middle space without fixed-coordinate drift or row-height regression. This is implementation/calibration evidence, not final Rule-9 acceptance: Foundation Lab is still the only runtime caller, so a non-Foundation production collection consumer must exercise the same contract before this shared-pattern slice closes.

The second bounded Shared Pattern + Interaction Kit implementation candidate now provides production `TrailCollectionRow` under `ui/patterns`. It owns generic dense-row geometry, divider/hover/highlight/selected presentation, one dedicated selection gutter, an independent semantic `leading` slot, and a min-width-safe content carrier. The caller supplies any selection control such as `TrailCheckbox`; CollectionRow neither creates nor stores selection state. The selection gutter remains structurally independent from semantic leading content: hover/highlight may reveal an unchecked selection control, and a selected row keeps the checked control visible without replacing Status, Priority, or another semantic glyph. Foundation Lab uses only local specimen state to exercise that presentation contract. Selection lifetime/scope, range/select-all, Action Registry, Bulk, and keyboard dispatch remain outside CollectionRow and belong to the later shared interaction owner.

Full local verification is green across 96 Vitest files / 329 tests, ESLint, TypeScript, production esbuild, and `git diff --check`. Representative real-Obsidian validation covers ordinary, hover/highlight, and selected states in wide and narrow panes: revealing the selection gutter does not shift semantic leading/content, selected state remains distinct from highlight, and the checkbox remains visible after pointer exit while the specimen remains selected. This is implementation/calibration evidence, not Rule-9 acceptance: Foundation Lab is still the only runtime consumer of CollectionRow, so a non-Foundation semantic collection row must exercise the same contract before the pattern is accepted.

The third bounded Shared Pattern + Interaction Kit implementation candidate now provides production `TrailPropertyControl` under `ui/patterns`. It is a native button-based presentation shell for compact property affordances with one proven `compact` density. It forwards native button/ARIA/interaction props and deliberately owns no Status/Priority/Estimate/Project semantics, picker/popover state, target legality, Query resolution, or Application mutation. Foundation Lab now consumes the production shell in both the standalone Property language specimen and the compact property strip inside the Composer specimen, proving the same presentation contract across two distinct compositions without promoting either Lab composition itself.

Full local verification is green across 97 Vitest files / 332 tests, ESLint, TypeScript, production esbuild, and `git diff --check`. Representative real-Obsidian validation covers the normal property panel and Composer in wide and narrower Trail panes: controls retain the established low-noise hierarchy, the compact Estimate `M` remains intrinsic rather than stretching to the normal control width, and the Composer property strip reflows without overflow or density regression. This is implementation/calibration evidence, not Rule-9 acceptance: Foundation Lab remains the only checked-in consumer graph, so a non-Foundation semantic property consumer must exercise the shell before PropertyControl is accepted.

This evidence is sufficient to accept the current Core Primitive Kit as the smallest independently justified primitive foundation. Badge/Tag, Tooltip, Popover, Menu, Select/Combobox, Dialog, Kbd, indeterminate/select-all treatment, and richer loading/disabled/destructive behavior remain real capabilities, but they are no longer required as speculative standalone primitives before shared-pattern work continues. They should appear only when the owning pattern/interaction or semantic consumer proves a stable reusable contract.
...
- **Calibrate** — current Linear evidence or real Obsidian host behavior shows a material deviation, so the owning token or correct native/Trail consumer is adjusted instead of appending a repair override;
- **Keep replaceable** — the decision needs a real production consumer or host constraint that does not yet exist, so the current value remains implementation-owned and explicitly non-global.

The accepted Foundation covers whole-shell surface hierarchy and density, semantic typography/color/state language, spacing/radius/hairline/elevation direction, icon sizing/stroke rhythm, representative native controls and menus, keyboard focus, hover/pressed/selected/disabled/destructive/text-selection distinctions, Markdown Live Preview/Reading/Properties presentation, and container-based visual responsiveness. Exact future glyph choices, Label palette assignment, component/page geometry, Inspector thresholds, production overlay motion, and Foundation Lab's current `620px` / `860px` calibration thresholds remain replaceable until their real consumers justify a contract.
...
4. Canonical stylesheet source lives under `plugin/styles/` in explicit ownership order: tokens -> Obsidian semantic mapping -> native consumers -> production primitive/pattern contracts -> Trail shell -> calibration-only Foundation Lab. `esbuild.config.mjs` uses one ordered manifest for both one-shot build and development watch to plain-concatenate those modules into the single generated `.obsidian/plugins/trail/styles.css` host artifact. A later module/override is not an ownership mechanism.
...
- [x] **2. Frontend Architecture + Foundation consolidation**
  ...
  - physically modularized canonical stylesheet source under `plugin/styles/` while preserving the same ownership/cascade order and one generated Obsidian `styles.css` delivery artifact;
  - passed complete local automated verification and representative real-host validation after the rewrite; the later source modularization additionally proved byte-identical generated stylesheet output and a shared build/watch source manifest.
- [x] **3. Visual Foundation acceptance**
  - accepted canvas/surface hierarchy/density, typography, accent/semantic colors, spacing/density direction, radius, hairlines/elevation, icon sizing/stroke rhythm, focus/hover/pressed/selected/disabled/destructive/text-selection states, restrained motion principle, and document/editor presentation;
  - validated the full enabled Obsidian application through representative Trail Lab/navigation, tabs/chrome, File Explorer, Settings controls, native Menu, Markdown Live Preview/Reading, Properties focus, and a narrow Trail split inside a still-wide window;
...
- [x] **4. Core Primitive Kit**
  ...
  - [x] third bounded slice: native Checkbox contract, with full local repository validation and representative real-host Collection density validation;
  - generic SegmentedControl is not accepted as a primitive; the real List/Board/Timeline layout switch is now owned by the reusable View Bar shared pattern instead;
  - the independently justified primitive foundation closes here; Badge/Tag, Tooltip, Popover, Menu, Select/Combobox, Dialog, Kbd, and richer shared states remain consumer-driven capabilities rather than mandatory empty primitives;
  - continue preferring Obsidian/browser/mature focused mechanics where suitable and expose small typed semantic variants over Trail tokens rather than arbitrary style props.
- [ ] **5. Shared Pattern + Interaction Kit**
  - [ ] View Bar candidate: responsive Filter/Display action slots and optional binary List/Board/Timeline layout switching are implementation/host-calibration green; final acceptance waits for the first non-Foundation production collection consumer;
  - [ ] CollectionRow candidate: generic dense-row presentation, independent selection gutter/semantic leading composition, and selected/highlight presentation are implementation/host-calibration green; final acceptance waits for the first non-Foundation semantic collection consumer;
  - [ ] PropertyControl candidate: compact property-action presentation plus the proven compact density are implementation/host-calibration green; final acceptance waits for the first non-Foundation semantic property consumer;
  - remaining Surface/Card foundations, Overlay/Composer shell, List/Board foundations, and common focus/selection mechanics beyond CollectionRow presentation;
...
8. Product Workspace / V1 UI Implementation             ACTIVE
   ...
   |- Visual Foundation acceptance                      CLOSED
   |- Core Primitive Kit                                CLOSED
   |- Shared Pattern + Interaction Kit                  IN PROGRESS
...
- Core Primitive Kit second slice **(Progress and Separator only; the rejected generic SegmentedControl abstraction was removed)**;
- Core Primitive Kit third slice **(native Checkbox contract; collection-selection behavior remains shared-pattern work and the independently justified primitive foundation is accepted)**.

The View Bar, CollectionRow, and PropertyControl implementation candidates are not yet counted as completed Gate 8 slices because their checked-in/runtime consumer graph still ends at Foundation Lab; Rule 9 acceptance waits for non-Foundation production consumers to exercise the contracts.

CSS source modularization / deterministic stylesheet build is a completed frontend-infrastructure alignment checkpoint, not a product/pattern acceptance slice. It changes canonical source ownership and build/watch mechanics only; the generated Obsidian stylesheet is byte-identical to the pre-migration stylesheet, so View Bar, CollectionRow, Visual Foundation, and Core Primitive acceptance status is unchanged.
...
This evidence establishes the frontend architecture/Foundation implementation baseline. It does **not** promote Lab markup into production APIs; the later Visual Foundation acceptance evidence below records which shared visual contracts are now stable enough for production primitives and which values deliberately remain replaceable.

### 6.10 Visual Foundation acceptance evidence

The Visual Foundation acceptance on top of public checkpoint `eb76b750ed4a25106e47897a38b2bcdc51055995` verifies:

- whole-shell hierarchy/density is accepted across Trail Navigation/Foundation Lab and representative Obsidian tabs/header/Ribbon/status/File Explorer surfaces, with work content remaining visually primary and navigation/chrome remaining quiet;
- typography is one semantic authority for Lab consumers, and the calibrated main-work hierarchy no longer loses visual priority to sidebar/supporting chrome;
- generic host-control styling preserves native mechanics: the Settings Language dropdown regression caused by resetting the native background image was repaired at the generic-control consumer, while desktop tabs were compacted through Obsidian's tab geometry variables rather than by replacing tab behavior;
- interaction states keep separate semantics for quiet hover, pressed, collection selection, keyboard focus, disabled, destructive, and text selection; text selection has its own owner rather than borrowing collection-selection state;
- representative keyboard validation distinguishes current/active File Explorer rows from keyboard-focused rows, restores visible native button `:focus-visible`, and keeps native composite Settings/Properties focus behavior intact where Obsidian owns the interaction surface;
- the native Obsidian context Menu remains on Obsidian mechanics while consuming the shared Trail menu visual contract; representative density, separator, icon/text alignment, submenu treatment, and destructive presentation are accepted without an exhaustive overlay rewrite;
- Markdown Live Preview and Reading View, Properties display/edit focus, and the surrounding whole-shell presentation remain visually coherent under one dark application system;
- Foundation Lab responds to its actual pane/container when the Trail split is narrow even while the overall Obsidian window remains wide, proving the container-based architecture without promoting the Lab's current `620px` / `860px` thresholds into global Trail breakpoints;
- palette/surface hierarchy, radius/border/elevation language, icon sizing/stroke rhythm, and restrained motion direction are stable enough for production primitives; exact production glyph choices, Label palette assignment, component/page geometry, Inspector thresholds, overlay motion details, and consumer-specific responsive thresholds remain replaceable until their real consumers exist;
This closes Visual Foundation acceptance without claiming every future component or page pixel is final. The independently justified Core Primitive Kit is now accepted, and the active Shared Pattern + Interaction Kit must continue to consume these accepted owners rather than copy Lab classes or reintroduce component-local visual facts.

### 6.11 Core Primitive Kit first-slice evidence

The first bounded Core Primitive Kit slice on top of public checkpoint `5c31492322c89b6f33b4437be410f129fff07a57` verifies:

- production `TrailButton`, `TrailIconButton`, `TrailInput`, and `TrailTextarea` owners now live under `plugin/src/ui/primitives`;
- Button variation remains intentionally narrow: the ordinary native button is the base contract and `primary` is the only public semantic variant because it maps to a frozen real consumer responsibility; Lab-only secondary/ghost specimens do not expand the production API;
- IconButton owns accessible icon-only button semantics and reuses Obsidian's `clickable-icon` shell, but does not introduce a separate icon registry/rendering subsystem;
- Input/Textarea forward native React contracts while withholding public `className`/`style` visual escape hatches;
- Foundation Lab consumes production primitives where it exercises their real contract while static calibration-only specimens remain Lab-owned;
- no new primitive stylesheet owner was added because accepted root native-control mappings already own these visual facts;
...
This closes only Progress and Separator as the second Core Primitive Kit slice. Badge/Tag where justified, Tooltip, Popover, Menu, Select/Combobox, Dialog, Kbd, and shared loading/disabled/destructive treatment remain open. A generic SegmentedControl is not counted as remaining primitive work unless later real consumers prove that ownership; the frozen layout-switch responsibility currently proceeds through the shared View Bar/pattern path.

### 6.13 Core Primitive Kit third-slice evidence

The third bounded Core Primitive Kit slice on top of public checkpoint `8193ef711edcd6d8a520c67c8b81138dba5f7f03` verifies:

- production `TrailCheckbox` now lives under `plugin/src/ui/primitives`;
- `TrailCheckbox` reuses the native `<input type="checkbox">` contract, requires an explicit accessible label, forwards the normal native input/ref/change/checked/defaultChecked/disabled/name/value contract, and withholds caller-owned `type`, `className`, `style`, and `aria-label` escape hatches;
- the primitive deliberately does not own row hover replacement, selected-row state, range/select-all behavior, Bulk state, or an `indeterminate` API before a real shared Selection consumer proves that responsibility;
- no new Checkbox CSS contract was added because the accepted Trail/Obsidian host mapping already renders the native checkbox with acceptable Linear-faithful density, alignment, and state weight; visual duplication is not introduced merely to create component-local CSS;
- Foundation Lab consumes the real primitive inside the Collection density specimen rather than as an isolated form specimen. At this historical primitive checkpoint the Lab used the checkbox as a stand-in for later collection-selection presentation; the later CollectionRow calibration supersedes that provisional slot arrangement with a dedicated selection gutter that remains independent from semantic leading content;
- the historical selected checkbox specimen was intentionally read-only because that checkpoint demonstrated primitive presentation rather than shared selection state; the later CollectionRow specimen now exercises persistent selected presentation with Lab-local state while the real shared Selection owner remains ahead;
- local full verification is green across ESLint, 94 Vitest files / 324 tests, TypeScript `tsc --noEmit`, production esbuild, and `git diff --check`;
- representative real-Obsidian validation shows the checkbox fits the existing leading slot without increasing row height or causing a material alignment/visual-weight regression.

This closes the native Checkbox primitive contract as the third Core Primitive Kit slice. That primitive checkpoint did not freeze how checkbox presentation composes with semantic row leading content. The later CollectionRow candidate now owns the dedicated selection gutter plus selected/highlight presentation; selection lifetime, deselection/range/select-all/Bulk behavior, and any justified indeterminate/select-all treatment remain outside the Checkbox primitive and CollectionRow state owner. Together with the first two slices, this is the accepted independently justified Core Primitive Kit foundation; later generic capabilities are introduced from real shared-pattern/semantic consumers rather than as prerequisite empty components.

### 6.14 Shared Pattern + Interaction Kit first-slice implementation-candidate evidence

The first bounded Shared Pattern + Interaction Kit implementation candidate published on top of public checkpoint `10292ef336658f451071e42d43b976fa90f06d82` verifies:

- production `TrailViewBar`, `TrailViewBarAction`, and `TrailViewLayoutSwitch` now live under `plugin/src/ui/patterns` and do not depend on Foundation Lab, but the current runtime consumer graph still ends at Foundation Lab; no non-Foundation production page/shell consumer invokes the pattern yet;
- `TrailViewBar` owns one collection-control plane with a flexible leading region and intrinsic trailing cluster, allowing Filter clauses/actions to grow independently while layout/Display remain a compact right-side group when capacity permits;
- List-only consumers omit the optional layout slot instead of rendering a disabled or empty switch, matching Initiative Focus, Triage, Historical Cycle, and non-Started Project Workspace responsibilities;
- `TrailViewLayoutSwitch` accepts exactly two caller-provided layout options, reuses `TrailIconButton`, and communicates the current layout through `aria-pressed`; it owns no Project/Cycle/Issue semantics and is not a revived generic SegmentedControl;
- production geometry is relative/adaptive: flex basis, intrinsic trailing width, `margin-left: auto`, and wrapping respond to the actual View Bar container. The contract has no fixed page coordinates or window-width media-query dependency;
- Foundation Lab consumes the production pattern as one complete View Bar and uses only local List/Board state to exercise interaction; Lab-only calibration icons remain caller-provided content rather than a production icon registry;
- local full verification is green across ESLint, 95 Vitest files / 326 tests, TypeScript `tsc --noEmit`, production esbuild, and `git diff --check`;
- representative real-Obsidian evidence covers a wide Trail pane and a materially narrower pane, with the user switching between List and Board. Filter remains the leading action, layout/Display remain one quiet trailing cluster, selected layout changes without a capsule/track abstraction, and narrowing does not produce fixed-coordinate drift, overlap, or row-height regression.

This does **not** close the first shared-pattern slice yet. The implementation candidate is automated-check green and host-calibrated, but Rule 9 requires the contract to survive outside Foundation Lab through a real production collection consumer. Filter popover/state, Display options, applied Filter clauses, concrete Projects/Cycle page state ownership, CollectionRow/Selection, List/Board content foundations, and the remaining interaction kit remain open; the first real collection consumer should consume this candidate rather than re-create its control plane.

### 6.15 Shared Pattern + Interaction Kit CollectionRow implementation-candidate evidence

The CollectionRow implementation candidate on top of public checkpoint `2a508106ad35b5d05f00adae09d069213e5f92dd` verifies:

- production `TrailCollectionRow` now lives under `plugin/src/ui/patterns` and Foundation Lab is still its only runtime consumer;
- the component owns generic dense-row geometry, divider/hover/highlight/selected presentation, one dedicated selection gutter, an independent semantic leading slot, and a min-width-safe content carrier without binding Project/Issue/Triage fields;
- the caller supplies any selection control, currently demonstrated with `TrailCheckbox`; CollectionRow does not store selection, derive selection from checkbox DOM state, synthesize entity identity, or own navigation/action semantics;
- selection presentation is structurally independent from semantic leading content. The checkbox is visually hidden by default, revealed in the left gutter for hover/highlight/focus, and remains visible while selected; Status, Priority, or another semantic leading glyph remains a separate slot and is never replaced by the checkbox;
- selection-control click/keydown is isolated from row activation so explicit selection affordances do not accidentally trigger ordinary row navigation/open behavior;
- Foundation Lab uses local boolean specimen state only to exercise the selected presentation lifecycle; that state is calibration scaffolding, not the shared Selection owner;
- range/select-all, selection lifetime/scope, Action Registry, Bulk target/intersection logic, keyboard dispatch, and collection-wide selection mechanics remain unimplemented here and stay assigned to the later shared interaction owner;
- local full verification is green across ESLint, 96 Vitest files / 329 tests, TypeScript `tsc --noEmit`, production esbuild, and `git diff --check`;
- representative real-Obsidian evidence covers wide and narrow Trail panes plus real pointer interaction: the selection gutter appears without shifting semantic leading or content, selected and highlighted surfaces remain distinct, and a checked specimen remains selected/visible after pointer exit until explicitly deselected.

This does **not** accept CollectionRow yet. The callable/presentation contract is automated-check green and host-calibrated, but Rule 9 requires a non-Foundation semantic collection consumer before acceptance. That future consumer must compose the generic row without forcing entity semantics, selection lifetime, or page-specific layout back into CollectionRow.

### 6.16 CSS source modularization / deterministic stylesheet-build evidence

The CSS source modularization checkpoint on top of public checkpoint `ecc2dbfc9e716e6e9b5cf969e5021223ef0ca594` verifies:

- the previous root `styles.css` source was split only at its established canonical owner boundaries into `plugin/styles/tokens.css`, `obsidian-variables.css`, `obsidian-native.css`, `primitives.css`, `patterns.css`, `shell.css`, and `foundation.css`;
- `esbuild.config.mjs` owns one explicit ordered `stylesheetSources` manifest. Production/diagnostics one-shot builds and development watch consume that same manifest, so file coverage and cascade order do not drift between modes;
- stylesheet assembly is plain UTF-8 concatenation with no `@import`, glob-order dependency, CSS preprocessor, minifier, parser transform, or added package dependency;
- `manifest.json` remains a static copied asset, while `.obsidian/plugins/trail/styles.css` is now a generated host delivery artifact. Root `styles.css` is removed so generated output cannot compete with canonical source ownership;
- before removing the root source, the locally generated stylesheet matched it exactly at SHA-256 `9F30841AD0BDF8A370ED70061C823FC4C29B21E32C33AE5D37BD57A451B0F24D`; the corresponding Git blob remains `d554c40374ed7b317b86d13677ee18250180b2a5`, proving byte-for-byte migration parity and therefore unchanged cascade/visual behavior;
- isolated pre-delivery verification exercised both one-shot production assembly and development-watch rebuild after a stylesheet-source event, with byte parity preserved;
- final local verification after root `styles.css` removal is green across ESLint, 96 Vitest files / 329 tests, TypeScript `tsc --noEmit`, production esbuild, and `git diff --check`.

This is a source/build-structure checkpoint, not a visual recalibration or a new Shared Pattern acceptance event. Because Obsidian receives identical stylesheet bytes, existing Visual Foundation/Core Primitive acceptance and View Bar/CollectionRow candidate evidence remain valid without another host screenshot pass; Rule 9 status is unchanged.

### 6.17 Shared Pattern + Interaction Kit PropertyControl implementation-candidate evidence

The PropertyControl implementation candidate published as `fa7dd25cc6cb48600c47ada3bf95486d924d1158` on top of public checkpoint `013be80dde562659c1a87f3220c7ea61b33931dd` verifies:

- production `TrailPropertyControl` now lives under `plugin/src/ui/patterns` and exposes one small native-button contract plus only the real-consumer-backed `compact` density;
- the pattern owns compact property-control presentation, hover/pressed/focus-visible/disabled styling, and intrinsic sizing; it does not own Status/Priority/Estimate/Project meaning, picker/popover state, legal targets, Query, or Application mutation;
- native button and ARIA props are forwarded, so later semantic property components/interactions can provide their own accessible picker state without adding a second low-level control mechanism;
- Foundation Lab consumes the same production owner in two distinct compositions: the labelled Property language panel and the compact property strip inside the Composer specimen. Lab-specific row grids, glyphs, semantic values, and Composer structure remain outside the production pattern;
- the earlier `trail-lab-property-control` visual owner has been removed from Foundation-only CSS; the reusable control contract now lives with production patterns while Foundation retains only its specimen composition;
- local full verification is green across ESLint, 97 Vitest files / 332 tests, TypeScript `tsc --noEmit`, production esbuild, and `git diff --check`;
- GitHub CI run #136 for the exact published head `fa7dd25cc6cb48600c47ada3bf95486d924d1158` completed successfully;
- representative real-Obsidian evidence covers wide and narrower Trail panes: normal controls preserve the established quiet property hierarchy, the compact Estimate control remains intrinsic around `M`, and Composer property controls reflow without overflow, height regression, or conversion into oversized buttons.

This does **not** accept PropertyControl yet. The callable/presentation contract is automated-check green and host-calibrated, but Rule 9 still requires a non-Foundation semantic property consumer. That consumer should supply semantic identity, picker/open state, capability legality, and mutation intent around this shell rather than pushing those responsibilities into PropertyControl.

### 6.18 General verification discipline
...
V1 implementation is ready for final product hardening only after the frozen Product/Domain/Data/UI answers are implemented through their canonical owners without temporary models, alternate persistence paths, duplicate mechanisms, or Page-private reconstructions. The exposed lower-layer semantic corrections through Milestone Progress, the UI Reset, frontend architecture/Foundation consolidation including deterministic modular stylesheet source/build ownership, Visual Foundation acceptance, and independently justified Core Primitive Kit foundation are closed. The legacy POC presentation has already left the active implementation; before final hardening, the accepted token/core-primitive foundation must now continue through the remaining pattern/interaction -> semantic component -> page/shell stack and the remaining product locations plus Default Project Settings/delete guidance. Cycle Start/Close/Start-next interaction/vocabulary, Milestone Progress semantics, the Required Default Project ready-state lower layer, Canonical Estimate alignment, and Workflow Issue Project-control legality are already closed.
