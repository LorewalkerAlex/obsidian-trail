...
| Markdown + Persistence Foundation | 建立共享 Markdown Core、Physical Schema / Registry、Source I/O / Repository；迁移 Triage / Project physical path | 当前 |
| Runtime + Mutation Foundation | 拆分 Runtime ownership；建立共享 projection / physical planning / single- and multi-source execution | 待开始 |
...
Architecture Contract 的文档 checkpoint、push 后 GitHub consistency review 与 authority-chain 校准已经完成；当前实现正式进入 **Markdown + Persistence Foundation**。
...
| **Implementation Architecture Re-baseline** | **当前** | Architecture Contract 已完成，当前进入 Markdown + Persistence Foundation |
