...
Initiative Focus remains owned by the existing Projects/query/UI boundaries and is now frozen as the standard Project collection scoped to one Initiative, List-only, with the Initiative dimension removed from grouping/filter because location already supplies it. Home content is likewise frozen as derived modules over existing Runtime/utility data; neither closure introduces new Domain or persistence authority. The remaining spatial question is the shared Workspace Grid / responsive composition. Custom Views and Favorites are deferred and therefore create no V1 implementation obligation until a later product/UI closure reactivates them.
