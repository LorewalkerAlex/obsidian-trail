...
function projectResult(
  ...
  for (const issue of issues) {
    ...
    sourceByIssueId[issue.id] = range;
  }
  return {
    contribution: {
      filePath: targetPath,
...
function createFixture(options: {
  ...
  reconcileProjectContribution(
    runtimeStore,
    currentProject.contribution as TrailProjectContribution,
  );
...
describe("Triage Accept planner", () => {
  it("creates a new Backlog identity and carries applicable enrichment but not Triage Due", () => {
    ...
    expect(result.plan.targetCreate.issue).toMatchObject({
      context: "workflow",
      createdAt: 4_000,
    ...
    expect(result.plan.targetCreate.issue.due).toBeUndefined();
    expect(result.plan.targetCreate.issue.id).not.toBe(fixture.source.id);
  });
});
