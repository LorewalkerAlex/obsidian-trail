...
export type TriageManagementPlanResult =
  | { readonly kind: "ready"; readonly plan: UpdateTriageIssuePlan | DeleteTriageIssuePlan }
...
export class TrailTriageManagementService {
  ...
  private verifyPersistedResult(
    plan: UpdateTriageIssuePlan | DeleteTriageIssuePlan,
    ...
    const persistedIssue = persisted.contribution.issuesById[
      plan.kind === "delete-triage-issue" ? plan.issueId : plan.issue.id
    ];
    const valid = plan.kind === "delete-triage-issue"
      ? persistedIssue === undefined
      : persistedIssue !== undefined && sameTrailTriageIssue(persistedIssue, plan.issue);
    if (!valid) {
      this.diagnostics.record("triage.validation.failed", {
        correlationId,
        data: {
          issueId: plan.kind === "delete-triage-issue" ? plan.issueId : plan.issue.id,
          kind,
