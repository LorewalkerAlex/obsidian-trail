...
describe("TrailFoundationLab pattern ownership", () => {
  it("shelves shared Page and collection patterns under Patterns while semantic Project rows stay under Semantic Entities", () => {
    render(<TrailFoundationLab control={{ kind: "ready" }} revision={7} />);

    const patterns = within(screen.getByRole("region", { name: "Patterns" }));
    expect(patterns.getByRole("group", { name: "Page header" })).toBeInTheDocument();
    ...
    expect(semanticEntities.queryByRole("group", { name: "Page header" })).not.toBeInTheDocument();
