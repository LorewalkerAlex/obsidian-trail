...
export type TrailMarkdownHeadingNode = Extract<
  TrailMarkdownRootChild,
  { type: "heading" }
>;
export type TrailMarkdownHtmlNode = Extract<
  TrailMarkdownRootChild,
  { type: "html" }
>;
...
export function readMarkdownLine(
  text: string,
  startOffset: number,
): TrailMarkdownLine {
...
export function markdownDataMarkerJson(
  node: TrailMarkdownRootChild | undefined,
): string | null {
  if (node?.type !== "html") {
    return null;
  }
...
export function isMarkdownDataMarker(
  node: TrailMarkdownRootChild,
): node is TrailMarkdownHtmlNode {
...
export function normalizeMarkdownRecordBody(
  markdown: string,
): string | undefined {
...
/**
 * Collects direct H2 records from one H1-owned root region. H3-H6, blockquotes,
 * fenced code, and ordinary HTML stay inside the current H2 record because mdast
 * exposes them as nested or non-heading root nodes rather than record boundaries.
 */
...
export function removeMarkdownRange(
  markdown: string,
  startOffset: number,
  endOffset: number,
): string {
...
/**
 * Replaces only the H2 heading line and immediate metadata marker. The record body
 * and every byte outside those two ranges remain untouched.
 */
export function replaceMarkdownHeadingAndMarker(
  ...
  let next = [
    markdown.slice(0, input.markerStartOffset),
    input.marker,
    markdown.slice(input.markerEndOffset),
  ].join("");
  next = [
    next.slice(0, input.recordStartOffset),
    input.heading,
