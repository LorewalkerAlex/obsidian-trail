...
export function createDiagnosticTrailUiActions(
  ...
  return {
      ...
      start(input): TrailEntityMutationReceipt {
        ...
        try {
          return observeReceipt(
            diagnostics,
            "ui.cycle.start",
            session.cycles.start(input),
            data,
          );
        } catch (error: unknown) {
          return recordThrown(diagnostics, "ui.cycle.start", error, data);
        }
      },
    },
    initiatives: {
    ...
    triage: {
      ...
      create(input): TrailEntityMutationReceipt {
        const data = {
          descriptionProvided: input.description !== undefined && input.description.trim() !== "",
          due: input.due ?? null,
          labelCount: input.labelIds.length,
        ...
        try {
          return observeReceipt(
            diagnostics,
            "ui.triage.create",
            session.triage.create(input),
            data,
          );
        } catch (error: unknown) {
          return recordThrown(diagnostics, "ui.triage.create", error, data);
        }
      },
