...
import type { TrailTriagePersistenceGateway } from "../../persistence/domain-sources/trail-triage-persistence";


export type { TrailTriagePersistenceGateway } from "../../persistence/domain-sources/trail-triage-persistence";
...
export class TrailTriageIntakeService {
  ...
  public capture(input: QuickCaptureInput): TriageCaptureReceipt {
    ...
    const { issue, plan: logicalPlan } = result;
    ...
    const completion = submitTrailMutation(
      ...
      this.mutationQueue,
      {
        ...
        optimisticData: { issueId: issue.id },
        plan: logicalPlan,
        ...
        settle: (persisted) => {
          ...
          if (persisted.contribution.issuesById[issue.id] === undefined) {
            this.diagnostics.record("triage.validation.failed", {
              correlationId,
              data: {
                issueId: issue.id,
            ...
            throw new TriageIntakeError(
              "verification-failed",
              `Persisted Triage source does not contain Issue ${issue.id}`,
            );
          }
